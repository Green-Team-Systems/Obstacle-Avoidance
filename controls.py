...
class PIDController(object):
    ...
    def altitude_control(self, altitude_cmd, vertical_velocity_cmd, altitude, vertical_velocity, attitude, dt, acceleration_ff = 0.0):
        ...
        lookahead = 0.5  # meters (Look ahead of where you are by half a meter)
        if (altitude - lookahead <= altitude_cmd + 0.1):
            reduce_thrust = 0.5
        elif (altitude + lookahead >= altitude_cmd):
            reduce_thrust = -0.5
        else:
            reduce_thrust = 0.0
        ...
        velocity_error = (error - self.last_error) / dt  # Create a derivative term
